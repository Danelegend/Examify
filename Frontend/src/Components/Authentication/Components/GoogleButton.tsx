...
const SignInWithGoogle = ({ title, SetResponseMessage, onSuccess }: { title: string, SetResponseMessage: (msg: string) => void, onSuccess: () => void }) =>  {
            ...
            res.json().then((data: SignInResponse) => {
                switch (res.status) {
                    ...
                    case 403:
                        ...
                        break
                    case 400:
                        SetResponseMessage("Problem with Google Login")
                        break
                    case 200:
    ...
    return (
        <button onClick={onButtonPressed} className="flex items-center bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
