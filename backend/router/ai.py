...
async def GetImage(conversation_id: int, message_id: int, token: Annotated[str, Security(HTTPBearer401())]) -> FileResponse:
...
async def PostConversationMessage(conversation_id: int, request: PostConversationMessageRequest, token: Annotated[str, Security(HTTPBearer401())]) -> PostConversationMessageResponse:
    ...
    response = post_conversation_message(conversation_id, request.message)
...
async def PostNewConversation(subject: Annotated[str, Form()],
    ...
    conversation_id, tutor_message = post_conversation(user_id, subject, topic, question, supporting_image)
